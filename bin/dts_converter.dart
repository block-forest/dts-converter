...
void main(List args) {
  ...
  if(source_library_name == DEFAULT_SOURCE_LIBRARY){
    print("Please provide a --source-library name equivalent to what you find under 'declare var' in the d.ts source file, for example 'Chart' or 'THREE'.");
    exit(1);
  }
...
''';
  ...
  if (sourceDir.existsSync()) {
    sourceDir.listSync(recursive: true, followLinks: false).forEach((FileSystemEntity entity) {
      if (FileSystemEntity.typeSync(entity.path) == FileSystemEntityType.FILE && extension(entity.path).toLowerCase() == ".ts") {
...
void _convert(String asFilePath) {
  ...
  //Write new file
  String dartFileNameFull = join(target_basedir, create_full_project ? "lib" : "" , create_full_project ? "src" : "", dart_library_name.toLowerCase(), dartFileName);
...
String _applyMagic(String f) {
   ...
   */
  f = f.replaceAllMapped(new RegExp("(\\s*)interface\\s+([A-Za-z0-9.]+)\\s+(extends\\s+\\w+)*\\s*\\{([^}]+)\\}\n"), (Match m) => "${m[1]}\n@anonymous\n@JS()\nclass ${m[2]} {\n\texternal factory ${m[2]} ${m[3] == null ? "" : "/* ${m[3]} */"}(\n\t\t{${m[4]}});\n${m[4]}\n}\n");
  ...
  f = f.replaceAllMapped(new RegExp("([A-Za-z0-9_]+)\\s*:\\s*([A-Za-z0-9_\\[\\]]*)\\s*([),])"), (Match m) => "${m[2]} ${m[1]} ${m[3]}");
  ...
  f = f.replaceAllMapped(new RegExp("\\(\\s*\\{([^}]*)\\}\\s*\\);"), (Match m) {
    ...
    intrnl = intrnl.replaceAllMapped(new RegExp("(Func[A-Za-z0-9_<>]+)\\s+(.*)\\s*\\(\\s*\\)"), (Match n) => "${n[1]} ${n[2]}");
    //replace ; with ,
  ...
  f = f.replaceAllMapped(new RegExp("\\}\\s*\\)\\s*;\\s*([^}]+)\\}"), (Match m) {
    ...
    String intrnl = m[1].replaceAllMapped(new RegExp("([A-Za-z0-9_<>]+)\\s*([A-Za-z0-9_<>]+)\\s*;"), (Match n) => "external ${n[1]} get ${n[2]};");
    intrnl = intrnl.replaceAllMapped(new RegExp("any(.*)\\(\\s*\\)"), (Match n) => "external Func0<dynamic> get ${n[1]}");

    //prefix all functions with 'external'
    intrnl = intrnl.replaceAllMapped(new RegExp("([A-Za-z0-9_<>]+\\s+[A-Za-z0-9_<>]+\\(.*\\))\\s*;"), (Match n) => "external ${n[1]};");

    //create setters
    intrnl = intrnl.replaceAllMapped(new RegExp("external\\s+([A-Za-z0-9_<>]+)\\s+get\\s+([A-Za-z0-9_]+)\\s*;"), (Match n) => "external ${n[1]} get ${n[2]};\n\t\texternal set ${n[2]}(${n[1]} v);");
  ...
  f = f.replaceAll(new RegExp("external\\s*factory\\s*[A-Za-z0-9_]+\\s*[A-Za-z0-9_\\/\\* ]*\\s*\\(\\s*\\{\\s*\\}\\);"), "");
  ...
  if(r1.hasMatch(f) && r2.hasMatch(f)){
    ...
    content.replaceAllMapped(new RegExp("([A-Za-z0-9_]+)\\s*:\\s*\\{([^\\}]*)\\}"), (Match m) {
      ...
      declaredClasses = declaredClasses.replaceAllMapped(new RegExp("([A-Za-z0-9_]+)\\s+([A-Za-z0-9_]+)\\s*;"), (Match n) => "\nexternal ${n[1]} get ${n[2]};");
    });


    f = f.replaceAllMapped(new RegExp("@anonymous\\s*(@JS\\(\\)\\s*class\\s+${lib_name}\\s*\\{)"), (Match m) => "${m[1]}\n\t${insert}\n");
...
void _setupArgs(List args) {
  ...
  argParser.addOption('target-library', defaultsTo: DEFAULT_TARGET_LIBRARY, help: 'The name of the library to be generated.', valueHelp: 'target-library', callback: (_dlibrary) {
    dart_library_name = _dlibrary;
  });
  argParser.addOption('source-library', defaultsTo: DEFAULT_SOURCE_LIBRARY, help: 'The name of the javascript library to convert.', valueHelp: 'source-library', callback: (_slibrary) {
    source_library_name = _slibrary;
  });
  argParser.addOption('source', abbr: 's', defaultsTo: DEFAULT_SOURCE_DIR, help: 'The path (relative or absolute) to the Typescript .d.ts source file(s) to convert.', valueHelp: 'source', callback: (_source_basedir) {
    source_basedir = _source_basedir;
  });
  argParser.addOption('target', abbr: 't', defaultsTo: DEFAULT_TARGET_DIR, help: 'The path (relative!) the generated Dart library will be written to. Usually, your Dart project\'s \'lib\' directory.', valueHelp: 'target', callback: (_target_basedir) {
    target_basedir = _target_basedir;
  });
  argParser.addFlag('create-project', abbr: 'c', negatable: false, defaultsTo: FULL_DART_PROJECT, help: 'Create full dart project with pubspec and library file.', callback: (_flibrary) {
